...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">تعديل طلب</h1>
            <p class="mt-2 text-sm text-gray-600">تعديل معلومات الطلب</p>
        </div>
...
<div class="bg-white shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <form action="{{ route('orders.update', $order) }}" method="POST" class="space-y-6" id="order-form">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="customer_address" id="customer_address" rows="3" required
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm @error('customer_address') border-red-300 @enderror">{{ old('customer_address', $order->customer_address) }}</textarea>
                    ...
                    <div>
                        ...
                        <div id="phone-numbers-container" class="space-y-2">
                            @foreach(old('customer_numbers', $order->customer_numbers) as $number)
                            <div class="flex gap-2">
                                <input type="text" name="customer_numbers[]" placeholder="رقم الهاتف" value="{{ $number }}" required
            ...
            <div class="border-b border-gray-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" step="0.01" name="delivery_fees" id="delivery_fees" value="{{ old('delivery_fees', $order->delivery_fees) }}" required min="0"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="total_amount" id="total_amount" value="{{ old('total_amount', $order->total_amount) }}" required min="0" readonly
                    ...
                    <div>
                        ...
                        <select name="payment_method" id="payment_method"
                            ...
                            <option value="InstaPay" {{ old('payment_method', $order->payment_method) == 'InstaPay' ? 'selected' : '' }}>InstaPay</option>
...
<script>
    ...
    // Load existing items
    const existingItems = @json(old('items', $order->items));
    if (existingItems && existingItems.length > 0) {
            ...
            itemDiv.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="items[${itemIndex}][product_id]" class="product-select block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm" onchange="updateProductInfo(this, ${itemIndex})" required>
                            <option value="">اختر منتج</option>
                            ${products.map(p => `<option value="${p.id}" data-name="${p.name}" data-price="${p.discounted_price || p.selling_price}" ${p.id == item.product_id ? 'selected' : ''}>${p.name}</option>`).join('')}
                        </select>
                        <input type="hidden" name="items[${itemIndex}][product_name]" class="product-name" value="${item.product_name || ''}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">الكمية</label>
                        <input type="number" name="items[${itemIndex}][quantity]" min="1" value="${item.quantity || 1}" required onchange="calculateTotal()"
                               class="item-quantity block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">السعر</label>
                        <input type="number" step="0.01" name="items[${itemIndex}][price]" min="0" value="${item.price || ''}" required onchange="calculateTotal()"
                               class="item-price block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">الحجم</label>
                        <input type="text" name="items[${itemIndex}][size]" value="${item.size || ''}"
                               class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">اللون</label>
                        <input type="text" name="items[${itemIndex}][color]" value="${item.color || ''}"
                               class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm">
                    </div>
                    <div class="flex items-end">
                        <button type="button" onclick="removeOrderItem(this)" class="w-full px-4 py-2 bg-red-50 text-red-600 rounded-md hover:bg-red-100 transition-colors">
        ...
        // Initialize with one item if no existing items
        addOrderItem();
    }
