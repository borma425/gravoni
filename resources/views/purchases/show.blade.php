...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">تفاصيل الشراء</h1>
            <p class="mt-2 text-sm text-gray-600">معلومات عملية الشراء</p>
        </div>
    ...
    <div class="px-4 py-5 sm:p-6">
        <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $purchase->product->name }}</dd>
            </div>
            <div>
            ...
            <div>
                <dt class="text-sm font-medium text-gray-500">سعر التكلفة</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ number_format($purchase->cost_price, 2) }} ج.م</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">إجمالي القيمة</dt>
                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ number_format($purchase->quantity * $purchase->cost_price, 2) }} ج.م</dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-1">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                        {{ $purchase->returned_quantity }} وحدة
                    </span>
                </dd>
            </div>
            @endif
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $purchase->created_at->format('Y-m-d H:i') }}</dd>
            </div>
        </dl>
    </div>
</div>
