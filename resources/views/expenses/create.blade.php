...
<div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div class="min-w-0">
        ...
        <p class="mt-2 text-sm text-gray-600">سجل المصاريف التشغيلية (إعلانات، شحن، إيجار...) لخصمها من الأرباح</p>
    </div>
...
<div class="max-w-2xl">
    <form action="{{ route('expenses.store') }}" method="POST" class="bg-white shadow rounded-lg p-6">
        @csrf
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="description" id="description" value="{{ old('description') }}" required placeholder="مثال: حملة إعلانية على فيسبوك"
            ...
            <div>
                ...
                <textarea name="note" id="note" rows="2" placeholder="أي تفاصيل إضافية"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 @error('note') border-red-500 @enderror">{{ old('note') }}</textarea>
                @error('note')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
