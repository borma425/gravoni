...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">تعديل محافظة</h1>
            <p class="mt-2 text-sm text-gray-600">تعديل معلومات المحافظة ورسوم الشحن</p>
        </div>
...
<div class="bg-white shadow rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <form action="{{ route('governorates.update', $governorate) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700">اسم المحافظة</label>
                <input type="text" name="name" id="name" value="{{ old('name', $governorate->name) }}" required
            ...
            <div>
                ...
                <input type="number" step="0.01" name="shipping_fee" id="shipping_fee" value="{{ old('shipping_fee', $governorate->shipping_fee) }}" required min="0"
