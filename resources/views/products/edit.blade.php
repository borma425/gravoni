...
<div class="mb-6 sm:mb-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="min-w-0">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">تعديل منتج</h1>
...
<form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    @csrf
    @method('PUT')
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" required class="mt-1.5 block w-full border border-gray-300 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 sm:text-sm @error('name') border-red-300 @enderror">
                    @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="sku" class="block text-sm font-semibold text-gray-700">رمز التخزين (SKU)</label>
                    <input type="text" name="sku" id="sku" value="{{ old('sku', $product->sku) }}" required class="mt-1.5 block w-full border border-gray-300 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 sm:text-sm @error('sku') border-red-300 @enderror">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="mt-1.5 block w-full border border-gray-300 rounded-lg shadow-sm py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $product->description) }}</textarea>
        ...
        <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
            ...
            <p class="text-xs text-slate-500 mb-4">أضف المقاسات وحدد مخزون كل لون. الألوان تظهر تلقائياً من القسم أعلاه.</p>

            <div class="bg-gradient-to-br from-slate-50 to-gray-50 border border-slate-200 shadow-sm rounded-xl p-5 mb-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5">المقاس</label>
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="mt-1.5 relative rounded-lg shadow-sm">
                        ...
                        <input type="number" step="0.01" name="selling_price" id="selling_price" value="{{ old('selling_price', $product->selling_price) }}" required min="0" dir="ltr" class="block w-full border border-gray-300 rounded-lg py-2.5 pl-3 pr-12 focus:ring-violet-500 focus:border-violet-500 sm:text-sm text-right @error('selling_price') border-red-300 @enderror">
                    </div>
                </div>
                <div>
                    ...
                    <div class="mt-1.5 relative rounded-lg shadow-sm">
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"><span class="text-gray-500 sm:text-sm">EGP</span></div>
                        <input type="number" step="0.01" name="discounted_price" id="discounted_price" value="{{ old('discounted_price', $product->discounted_price) }}" min="0" dir="ltr" class="block w-full border border-gray-300 rounded-lg py-2.5 pl-3 pr-12 focus:ring-violet-500 focus:border-violet-500 sm:text-sm text-right @error('discounted_price') border-red-300 @enderror">
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6 sticky top-6">
            <h2 class="text-lg font-bold text-gray-900 mb-4">حفظ التعديلات</h2>
...
<script>
    ...
    function getDefinedColorNames() {
        const names = [];
        document.querySelectorAll('#colors-container [data-color-name]').forEach(card => { names.push(card.getAttribute('data-color-name')); });
        return names;
    }
    ...
    function refreshSizeColorStocks() {
        ...
        if (!colors.length) {
            container.innerHTML = '<p class="text-xs text-slate-400 italic" id="no-colors-hint">أضف ألوان أولاً من قسم "الألوان المتاحة" أعلاه</p>';
            return;
        }

        const currentStocks = {};
        container.querySelectorAll('[data-stock-color]').forEach(inp => { currentStocks[inp.getAttribute('data-stock-color')] = inp.value; });
    ...
    function editSize(btn) {
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < colorInputs.length; i++) {
                const stockInp = document.querySelector(`#size-color-stocks [data-stock-color="${colorInputs[i].value}"]`);
                if (stockInp) stockInp.value = stockInputs[i].value;
            }
        }, 50);
    ...
    (function loadExistingColors() {
        let colors = @json($product->available_colors ?? []);
        if (colors && !Array.isArray(colors)) colors = Object.values(colors);
    ...
    (function loadExistingSizes() {
        let sizes = @json($product->available_sizes ?? []);
        if (sizes && !Array.isArray(sizes)) sizes = Object.values(sizes);
        if (!Array.isArray(sizes)) return;

        sizes.forEach(s => {
            if (!s || !s.size) return;
            const colors = (s.colors || []).map(c => ({ color: c.color || '', stock: parseInt(c.stock) || 0 }));
            addSizeCard({
                size: s.size,
                chest: s.chest_width_cm || '',
                wMin: s.weight_kg?.min || '',
                wMax: s.weight_kg?.max || '',
                hMin: s.height_cm?.min || '',
                hMax: s.height_cm?.max || '',
                colors: colors
            }, sizeIndexCounter++);
        });
    })();
